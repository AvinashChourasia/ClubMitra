...
export function useRunRecorder() {
  ...
  const start = useCallback(async () => {
    ...
    if (isExpoGo) {
      ...
      ticker.current = setInterval(() => setElapsedS(Math.floor((Date.now() - startMs.current) / 1000)), 1000);
  ...
  // stop() ends recording and returns the captured track for the screen to
  // upload. Async because the background engine reads its track from storage.
  const stop = useCallback(async (): Promise<RunPoint[]> => {
    cleanup();
    setStatus("idle");
    return background.current ? await stopRun() : points.current;
  }, [cleanup]);
